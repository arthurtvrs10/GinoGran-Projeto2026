...
export default function GinogranPremiumCarousel() {
  ...
  const googleMapsUrl = "https://www.google.com/maps/search/?api=1&query=GinoGran+Mármore+Fabrica";
  ...
  const renderStars = (count: number = 5, size: string = "w-3 h-3") => {
    return (
      <div className="flex text-yellow-500 gap-[1px]">
        {[...Array(5)].map((_, i) => (
          <svg key={i} className={`${size} ${i < count ? 'fill-current' : 'text-gray-300 dark:text-gray-600'}`} viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth={i < count ? "0" : "1"}>
      ...
      <div className="max-w-5xl mx-auto px-6 grid grid-cols-1 md:grid-cols-4 gap-8 items-center">
        ...
        <div className="md:col-span-1 border-l-4 border-primary pl-4 flex flex-col gap-3 items-start">
          <div>
            ...
            <span className="text-[10px] text-primary font-bold uppercase tracking-widest">GinoGran</span>
          </div>
          ...
          >
            ...
            <div className="flex flex-col">
              ...
              <div className="flex items-center gap-1">
                 <span className="text-xs font-bold text-black-100 dark:text-white">4.9</span>
        ...
        <div className="md:col-span-3 relative flex items-center group">
          ...
          <div className="absolute inset-y-0 -left-4 -right-4 flex justify-between items-center opacity-0 group-hover:opacity-100 transition-opacity pointer-events-none">
            <button onClick={prev} className="pointer-events-auto p-1 text-black-30 hover:text-primary transition-colors hover:scale-110">
              <svg width="24" height="24" fill="none" stroke="currentColor" strokeWidth="2" viewBox="0 0 24 24"><path d="M15 19l-7-7 7-7"/></svg>
            </button>
            <button onClick={next} className="pointer-events-auto p-1 text-black-30 hover:text-primary transition-colors hover:scale-110">
